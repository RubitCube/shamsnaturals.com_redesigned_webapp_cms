...
const AdminBanners = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingBanner) {
        const response = await adminAPI.banners.update(
          editingBanner.id,
          submitData
        );
      ...
      <div className="grid grid-cols-3 gap-6">
        {banners.map((banner) => {
          ...
          if (banner.image_path?.startsWith("http")) {
            ...
            if (
              banner.image_path.includes("/storage/banners/") &&
              !banner.image_path.includes("/backend/public/")
            ) {
              const filename = banner.image_path.split("/storage/banners/")[1];
              imageUrl = `${siteUrl}/backend/public/storage/banners/${filename}`;
            } else {
              imageUrl = banner.image_path;
            }
          } else {
            imageUrl = `${BASE_URL}/public/storage/${banner.image_path
              ?.replace(/^storage\//, "")
              .replace(/^\//, "")}`;
          }
          return (
