...
import { pagesAPI, bannersAPI, adminAPI } from '../services/api'
...
const AboutPage = () => {
  ...
  const { mainImageSrc, decorativeImage1, decorativeImage2, contentWithoutImages, imageSeo } = useMemo(() => {
    const result = { 
      mainImageSrc: '', 
      decorativeImage1: '', 
      decorativeImage2: '',
    ...
    if (page?.seo?.image_seo) {
      result.imageSeo = {
        ...
        decorative_image_1: page.seo.image_seo.decorative_image_1 || { alt: '', title: '' },
        decorative_image_2: page.seo.image_seo.decorative_image_2 || { alt: '', title: '' },
      }
    }
          ...
          {page ? (
            <div className="relative pb-32">
              <div className="flex flex-wrap -mx-4 relative">
                ...
                <div className="float-right py-8 xl:py-4 animated animatedFadeInUp fadeInUp">
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: contentWithoutImages || page.content }}
                  />
                </div>
                </div>
                ...
                {mainImageSrc && (
                  <div className="w-full xl:w-5/12 2xl:w-6/12 px-0 text-center hidden xl:block xl:absolute xl:right-0 xl:top-[40%] py-5 my-auto z-10">
                    <div className="aboutbg relative">
                      <img 
                        src={mainImageSrc.startsWith('http') ? mainImageSrc : `http://localhost:8000/storage/${mainImageSrc}`}
              ...
              {(decorativeImage1 || decorativeImage2) && (
                <div className="hidden 2xl:block absolute bottom-0 left-0 w-full pointer-events-none z-0" style={{ height: '300px' }}>
                  {/* about02 - Upper outline illustration (tote bag) */}
                  {decorativeImage1 && (
                    <img 
                      src={decorativeImage1.startsWith('http') ? decorativeImage1 : `http://localhost:8000/storage/${decorativeImage1}`}
                  ...
                  {decorativeImage2 && (
                    <img 
                      src={decorativeImage2.startsWith('http') ? decorativeImage2 : `http://localhost:8000/storage/${decorativeImage2}`}
